...
public class MicrosoftOAuthController : ControllerBase
{
    ...
    {
        if (_stateStore == null || _msSettings == null)
        {
        ...
        if (!string.IsNullOrEmpty(accountId))
        {
            ...
            {
                _logger.LogWarning(
                    "Rejected Microsoft OAuth authorize-url for accountId {AccountId}: not owned by tenant {TenantId} user {UserId}",
    ...
    public async Task<IActionResult> HandleCallback([FromQuery] string? code, [FromQuery] string? state, [FromQuery] string? error)
    {
        ...
        {
            _logger.LogError("Microsoft OAuth provider returned an error: {Error}", error);
        ...
        var flow = await _stateStore.TryConsumeAsync(state, "microsoft");
        if (flow == null)
        {
            _logger.LogWarning("Microsoft OAuth callback rejected: state invalid, expired or already used");
            return RedirectToError("state_invalid");
        }
    ...
    public async Task<IActionResult> RefreshToken([FromBody] RefreshTokenRequest request)
    {
        ...
        try
        {
            ...
            {
                _logger.LogWarning(
                    "Rejected Microsoft OAuth refresh-token for accountId {AccountId}: not owned by tenant {TenantId} user {UserId}",
    ...
    private async Task SaveAccountWithTokens(OAuthFlowState flow, string userEmail, TokenResponse tokenResponse)
    {
        ...
        {
            throw new ArgumentNullException(nameof(flow.TenantId));
        }
        ...
        var account = accountId.IsNullOrWhiteSpace()
            ? await _accountStore.GetByEmailAsync(userEmail, flow.TenantId, MailProviderType.OutlookApi)
